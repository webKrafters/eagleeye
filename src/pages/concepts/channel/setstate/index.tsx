...
const ConceptChannelSetStatePage : React.FC<{className? : string}> = ({ className }) => (
    <article className={ `concept-channel-setstate-page ${ className }` }>
        ...
        <h3>About the channel setState method</h3>
        <blockquote style={{ margin: '1em 40px' }}>
            [This channel's] internal state is <strong><u>immutable</u></strong> and <strong><u>private</u></strong>.<br />
        ...
        <p>And to overwrite a slice of state, use the <Anchor to="/concepts/channel/setstate/tags">tag</Anchor> command.</p>
        ...
        <p>This can be achieved by collecting a series of state changes in an array and passing that array as an argument to the <code>channel.setState</code> method.</p>
        ...
        <NotePad>The channel also recognizes and resolves negative indexes when present in the indexed state change object. See additional <Anchor to="/concepts/channel/setstate#neg-idx-tip">tip</Anchor> below.</NotePad>
        ...
        <div>Please see full info on tag commands <Anchor to="/concepts/channel/setstate/tags">here</Anchor>.</div>
    </article>
);

export default ConceptChannelSetStatePage;
