...
const ConceptPrehooksPage : React.FC<{className? : string}> = ({ className }) => (
    <article className={ `concept-prehooks-page ${ className }` }>
        ...
        <div>
            ...
            <h3>Why Prehooks?</h3>
            <ul>
                <li>Prehooks provide a central place for sanitizing, modifying, transforming, validating etc. all related incoming state updates. The context store obtains its prehooks via its context <Anchor to="/concepts/provider">Provider's</Anchor> <code>prehooks</code> optional prop.</li>
