...
const ExternalAccessPage : React.FC<{className : string}> = ({ className }) => (
    <article className={ `external-access-page ${ className }` }>
        ...
        <ol id="external-apis">
            ...
            <li><strong><code>resetState</code>:</strong> Please see descriptions in the <Anchor to="/concepts/channel/resetstate">store</Anchor> page. May accept a parameterless invocation resulting in a noop.</li>
            <li><strong><code>setState</code>:</strong> Please see descriptions in the <Anchor to="/concepts/channel/setstate">store</Anchor> page.</li>
            <li >
        ...
        <Alert title="Pro Tips">
            ...
            <ListItem><div><code>stateManager.store.getState(...)</code> must be used to obtain the current state value. Please rememeber to join this storeManager's <Anchor to="/getting-started/#streaming">change stream</Anchor> instead if real-time change monitoring is what you need.</div></ListItem>
